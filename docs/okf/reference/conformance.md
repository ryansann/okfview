...
2. Every such block has a non-empty [`type`](/reference/frontmatter-schema.md).
...
okfview honors this strictly: the [OKF core](/architecture/okf-core.md) never throws and
always produces a [Bundle](/reference/data-model.md), surfacing problems as
[diagnostics](/features/diagnostics.md) instead. The `validate_bundle` and
`validate_document` [MCP tools](/reference/mcp-tools.md) return these issues with suggested
