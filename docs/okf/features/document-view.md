...
type: Feature
title: Document View
description: Renders a concept as styled Markdown with a first-class frontmatter header, internal navigation, and a backlinks rail.
...
highlighting) with a header built from the [frontmatter](/reference/frontmatter-schema.md):
a colored type pill, title, description, tags, timestamp, and a launchable `resource` link.
